...
from src.codec.utils import (
    load_hparams, build_env, load_checkpoint,
    save_checkpoint, get_state_dict,
...
from src.vocos.loss import SingleMelLoss
...
def train(rank, local_rank, world_size, h, resume_from_checkpoint: str = ""):
    ...
    mel_spectrogram = MelSpectrogramFeatures(sample_rate=h.sampling_rate, n_fft=h.n_fft, win_length=h.win_size, hop_length=h.hop_size, n_mels=h.num_mels, padding=h.mel_padding).to(device)
    mel_loss = SingleMelLoss(sample_rate=h.sampling_rate, n_fft=h.n_fft, win_length=h.win_size, hop_length=h.hop_size, n_mels=h.num_mels).to(device)
    ...
    optim_d = torch.optim.AdamW(itertools.chain(msd.parameters(), mpd.parameters(), mrd.parameters()), h.learning_rate, betas=[h.adam_b1, h.adam_b2])
    ...
    scheduler_d = torch.optim.lr_scheduler.OneCycleLR(
        optim_d, max_lr=h.learning_rate, total_steps=total_steps,
        pct_start=h.pct_start, div_factor=h.div_factor,
        final_div_factor=h.final_div_factor, anneal_strategy='cos',
        last_epoch=-1
    )
    ...
    while steps < h.max_training_steps:
        ...
        if steps >= h.start_gan_steps :
            ...
            optim_d.zero_grad()
            # msd
            real_score_msd, gen_score_msd, _, _ = msd(mel, mel_g.detach()) # Detach to prevent gradients from flowing back to the generator.
            loss_msd, loss_msd_real, _ = discriminator_loss(real_score_msd, gen_score_msd)
            loss_msd = loss_msd / len(loss_msd_real) # Average loss.

            # mpd
            real_score_mpd, gen_score_mpd, _, _ = mpd(y, y_g.detach()) # Detach to prevent gradients from flowing back to the generator.
            loss_mpd, loss_mpd_real, _ = discriminator_loss(real_score_mpd, gen_score_mpd)
            loss_mpd = loss_mpd / len(loss_mpd_real) # Average loss.

            # mrd
            real_score_mrd, gen_score_mrd, _, _ = mrd(y, y_g.detach()) # Detach to prevent gradients from flowing back to the generator.
            loss_mrd, loss_mrd_real, _ = discriminator_loss(real_score_mrd, gen_score_mrd)
            loss_mrd = loss_mrd / len(loss_mrd_real) # Average loss.
...
def main():
    ...
    # Determine distributed context (torchrun injects these env vars)
    ...
    if rank == 0:
        ...
        # Save config snapshot + resolved config
        try:
    ...
    # Override checkpoint output dir to runs/
